...
export class GLAD {
  ...
  generateDependenciesFromSwiftFiles () {
    ...
    let files = this.findFilesRecursively(startingPath, '.swift')

    // Apply exclude patterns if specified
    if (this.options.exclude) {
      const excludePattern = this.options.exclude
      files = files.filter(file => {
        // Convert exclude pattern to a relative path for matching
        const relativePath = path.relative(startingPath, file)
        // Simple glob matching - check if the exclude pattern matches
        return !this.matchesExcludePattern(relativePath, excludePattern)
      })
    }

    // Process Swift files with custom logic
    this.processSwiftFiles(files)
  }

  /**
   * Custom processing for Swift files
  ...
  processSwiftFiles (files) {
    ...
    files.forEach(file => {
      ...
      // Remove comments and strings from content before searching for definitions
      const uncommentedContent = this.removeCommentsAndStrings(content)
      ...
      let match
      while ((match = defRegex.exec(uncommentedContent)) !== null) {
        const name = match[1]
    ...
    files.forEach(file => {
      ...
      // Remove comments and strings from content before finding usages (to avoid false positives from commented code)
      const uncommentedContent = this.removeCommentsAndStrings(content)
      ...
      let match
      while ((match = usageRegex.exec(uncommentedContent)) !== null) {
   ...
   * @param {string} excludePattern - Glob pattern to match against
   * @returns {boolean} True if the file should be excluded
   */
  matchesExcludePattern (filePath, excludePattern) {
