...
export class FlutterDartParser {
  ...
  ensureContainmentAndLinkTheseTwo (containerName, sourceName, version, targetName) {
    ...
    this.glad.graph.upsertFileLinkByText(sourceName, targetName)
  }
  ...
  loadGraphFromFlutterDependencies (pubspecObject) {
    ...
    if (this.glad.options.externals === false) {
      const container = this.glad.graph.rootNode.getByText('transitive')
      if (container) {
        this.glad.graph.dropContainerAndInternalNodesAndLinks(container)
      }
    }

    if (this.glad.options.dev === false) {
      const container = this.glad.graph.rootNode.getByText('dev')
      if (container) {
        this.glad.graph.dropContainerAndInternalNodesAndLinks(container)
      }
    }
  ...
  upsertInContainer (containerName, nodeName, nodeVersion = null) {
    let containerFound = this.glad.graph.rootNode.getByText(containerName)
    if (!containerFound) {
      containerFound = this.glad.graph.rootNode.upsert(containerName)
    }
