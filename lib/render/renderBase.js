...
import { plural, stringIsEmpty, XMLTree } from '../util/strings.js'
import {
  getSvgCircle,
...
/**
 * @property {string[]} svgDoc - array of SVG fragments that will be joined to form the final SVG text representation
 */
export class RenderBase {
   ...
   * @param {Edge} edge - the edge object to update flags on
   */
  addLinePointer (pointSource, pointTarget, isCircular = false, tooltip = '', edge = null) {
    const x1 = pointSource.x
    const y1 = pointSource.y
    const x2 = pointTarget.x
    const y2 = pointTarget.y

    let className = 'line' // default class

    if (isCircular) {
      className = 'lineRed' // red edges
      if (edge) {
        edge.isUpDependency = false // circular edges are not up dependencies
      }
    } else {
      if (y1 < y2) {
        if (this.options.lines === Constants.LINES_WARNINGS) {
          ...
          if (edge) {
            edge.isUpDependency = false
          }
          return
        }
        className = 'line' // green edges (default)
        if (edge) {
          edge.isUpDependency = false
        }
      } else {
        className = 'lineOrange' // orange edges
        if (edge) {
    ...
    if (this.options.lineEffect.includes('outline')) {
      additionalAttributes = 'filter=\'url(#outlineBlack)\''
    }
    if (this.options.lineEffect.includes('shadow')) {
      if (additionalAttributes) additionalAttributes += ' '
      additionalAttributes += 'filter=\'url(#shadow)\''
    }

    switch (this.options.lines) {
      case Constants.LINES_ANGLE:
        this.svgDoc.push(getSvgConnectionAngle(x1, y1, x2, y2, null, className, additionalAttributes, tooltip))
        break
      case Constants.LINES_ELBOW:
        this.svgDoc.push(getSvgConnectionLine(x1, y1, x2, y2, null, className, additionalAttributes, tooltip))
        break
      case Constants.LINES_STRAIT:
        this.svgDoc.push(getSvgLine(x1, y1, x2, y2, null, className, additionalAttributes, tooltip))
        break
      case Constants.LINES_CURVE:
      default:
        this.svgDoc.push(getSvgCurve(x1, y1, x2, y2, null, className, additionalAttributes, tooltip))
        break
    }
  }
  ...
  addPillCounterXY (x, y, value, backgroundColor = '#000011aa', fontColor = 'white', tooltip = '') {
    ...
    this.svgDoc.push(getSvgCircle(x, y, (Constants.pillSize / 2), null, backgroundColor))
    this.svgDoc.push(`<text x="${x}" y="${y}" fill="${fontColor}" font-size="10" text-anchor="middle" dominant-baseline="central">`)
    this.svgDoc.push(value)
    this.svgDoc.push('</text>')
    if (!stringIsEmpty(tooltip)) {
      this.svgDoc.push('<title>' + tooltip + '</title>')
    }
  ...
  addRectXYWH (x, y, w, h, className = null, backgroundColor = '#aaaaff77', borderColor = null, id = null, roundCorner = 0, additionalAttribute = '') {
    ...
    this.svgDoc.push(getSvgRectangle(x, y, w, h, roundCorner, roundCorner, className, borderColor, backgroundColor, additionalAttribute))
  }

  addSvgStylesAndDefinitions () {
    this.svgDoc.push(`<style>
    .layerRectangle {
      stroke: black;
    ...
    this.svgDoc.push('<defs id=\'idSvgDef\'>')
    this.svgDoc.push('<linearGradient id="layers" gradientTransform="rotate(90)"><stop offset="0%" stop-color="#691872" stop-opacity="0.1"/><stop offset="100%" stop-color="#691872" stop-opacity="0.3" /></linearGradient>')
    this.svgDoc.push(`<filter id="shadow">
    ...
    this.svgDoc.push(`<filter id="outlineWhite">
    ...
    this.svgDoc.push(`<filter id="outlineBlack">
        <feMorphology in="SourceAlpha" result="DILATED" operator="dilate" radius="2"/>
    ...
    this.svgDoc.push('<marker id=\'lineEnd\' viewBox= \'0 -5 30 10\' refX=\'12\' orient=\'auto\'>')
    this.svgDoc.push('<path d=\'M0,-16L32,0L0,16\' fill=\'#3058FF\' opacity="0.67"/>')
    this.svgDoc.push('</marker>')

    this.svgDoc.push('<marker id=\'lineStart\' viewBox= \'0 -4 8 8\' refX=\'4\' orient=\'auto\'>')
    this.svgDoc.push('<circle cx=\'4\' r=\'4\' fill=\'#377E22\'/>')
    this.svgDoc.push('</marker>')

    this.svgDoc.push('</defs>')
  }
   ...
   */
  getSvgDocument () {
    return XMLTree(this.svgDoc.join(''))
  }
  ...
  renderEdgesOfLeaves (edges) {
    ...
    edges.forEach(edge => {
      ...
      this.addLinePointer(sourceCenter, targetCenter, edge.isCircular, tooltip, edge)
    })
    this.renderSvgGroupEnd()
  }
  ...
  renderLabels (node) {
    ...
    if (node.isLeaf) {
      ...
      this.svgDoc.push(`<text class="nodeName" x="${r.centerHorizontal}" y="${r.centerVertical}" font-size="${fontSize}">${containerTitle}</text>`)
      if (node.versionToString() !== '') {
        this.svgDoc.push(`<text class="nodeName" x="${r.centerHorizontal}" y="${r.centerVertical + 20}" font-size="80%">${node.versionToString()}</text>`)
      }
    } else {
      this.svgDoc.push(`<text class="folderName" x="${r.x + Constants.paddingText}" y="${r.y + Constants.paddingText}">${containerTitle}</text>`)
    }
    ...
    if (this.options.details) {
      ...
      if (!stringIsEmpty(textInfo)) {
        this.svgDoc.push(`<text x="${r.x + Constants.paddingText}" y="${r.bottom - Constants.paddingText}" fill='black' font-size="75%">`)
        this.svgDoc.push(`${textInfo}`)
        this.svgDoc.push('</text>')
      }
    }
  ...
  renderLayer (layer, text, index, showRectangle = true, showNumber = false) {
    ...
    if (showNumber || this.options.debug) {
      ...
      this.svgDoc.push(`<text x="${layer.rect.right - Constants.paddingText}" y="${layer.rect.top + Constants.paddingText}" class='layerText' dominant-baseline="hanging" text-anchor="end" >${index + 1} ${text}</text>`)
    }
    this.renderSvgGroupEnd()
  }
  ...
  renderNode (node) {
    ...
    // Check if node is orphan for styling
    const orphanNodes = this.graph.getOrphanNodes()
    const isOrphan = orphanNodes.includes(node)

    // The shape
    if (node.isLeaf) {
      // File - use orphan class if orphan, otherwise regular class
      const className = isOrphan ? 'nodeFileOrphan' : 'nodeFile'
      this.svgDoc.push(getSvgRectangle(r.x, r.y, r.w, r.h, 8, 8, className))
    } else {
      // Folder - currently no orphan styling for containers, use regular class
      this.svgDoc.push(getSvgRectangle(r.x, r.y, r.w, r.h, 2, 2, 'nodeFolder'))
    }
  ...
  renderSvgEnd () {
    // Close the translation group that was opened in renderSvgStart
    this.svgDoc.push('</g>')
    this.svgDoc.push('</svg>')
  }
  ...
  renderSvgGroupStart (id = null, className = null) {
    ...
    this.svgDoc.push('<g ' + idText + ' ' + (className || '') + '>')
  }
   ...
   */
  renderSvgStart (w = 1000, h = 1000, padding = 50) {
    // Add padding to prevent tooltip clipping at edges
    const totalWidth = w + (padding * 2)
    const totalHeight = h + (padding * 2)

    this.svgDoc.push('<?xml version="1.0" standalone="no"?>')
    this.svgDoc.push(`<svg width="${totalWidth}" height="${totalHeight}" viewBox="0 0 ${totalWidth} ${totalHeight}" xmlns="http://www.w3.org/2000/svg" font-family="Arial, Helvetica, sans-serif">`)

    // Shared style
    this.addSvgStylesAndDefinitions()

    // White background
    this.svgDoc.push('<rect width="100%" height="100%" fill="white"/>')

    // Create a group to offset the content by the padding amount
    this.svgDoc.push(`<g transform="translate(${padding}, ${padding})">`)
  }
}
...
export function createLayers (graph, layers, nodes, options) {
  if (nodes.length > 0) {
    ...
    // Performance optimization: limit retries based on graph size
    const maxRetries = Math.min(99, Math.max(10, Math.floor(nodes.length / 10)))
    let lastLayerChecksum = ''
    let retries = 0
    let stableIterations = 0
    const maxStableIterations = 3 // Allow 3 stable iterations before stopping

    layers.setAllLayersToTheSameMaxWidth()
    while (retries++ < maxRetries && stableIterations < maxStableIterations) {
      moveNodesToLayers(graph, layers, nodes)
      layers.compactEmptyLayers()
      layers = layers.sortByIndex()

      const currentChecksum = layers.toString()
      if (lastLayerChecksum === currentChecksum) {
        stableIterations++
      } else {
        stableIterations = 0
        lastLayerChecksum = currentChecksum
      }
    }
  ...
  const { h, w } = layers.placeLayersAndNodes(options.align)

  return { h, layers, nodesToRender: nodes, w }
}
...
function firstLayerStartingFromTheBottomThatCallsThisNode (graph, layers, node) {
  for (let layerIndex = layers.length - 1; layerIndex >= 0; layerIndex--) {
    ...
    const compareResult = graph.getOutInCached([node], peerNodesOnThatLayer)
...
function setInLayerOrInsertIfNeeded (graph, layers, node, layerIndex) {
  if (layerIndex >= 0 && layerIndex < layers.length) {
    ...
    const compareResult = graph.getOutInCached([node], layerToTestIfAllowed.nodes)
