...
export class RenderBase {
  ...
  addLinePointer (pointSource, pointTarget, isCircular = false, tooltip = '', edge = null) {
    ...
    switch (this.options.lines) {
      case Constants.LINES_ANGLE:
        this.svgDoc += getSvgConnectionAngle(x1, y1, x2, y2, null, className, additionalAttributes, tooltip)
        break
      case Constants.LINES_ELBOW:
        this.svgDoc += getSvgConnectionLine(x1, y1, x2, y2, null, className, additionalAttributes, tooltip)
        break
      case Constants.LINES_STRAIT:
        this.svgDoc += getSvgLine(x1, y1, x2, y2, null, className, additionalAttributes, tooltip)
        break
      case Constants.LINES_CURVE:
      default:
        this.svgDoc += getSvgCurve(x1, y1, x2, y2, null, className, additionalAttributes, tooltip)
        break
    }
  }
  ...
  renderNode (node) {
    ...
    if (node.isLeaf) {
      ...
      this.svgDoc += getSvgRectangle(r.x, r.y, r.w, r.h, 8, 8, className)
    } else {
  ...
  renderSvgStart (w = 1000, h = 1000, padding = 50) {
    ...
    this.svgDoc += `<svg width="${totalWidth}" height="${totalHeight}" viewBox="0 0 ${totalWidth} ${totalHeight}" xmlns="http://www.w3.org/2000/svg" font-family="Arial, Helvetica, sans-serif">`
    ...
    this.svgDoc += `<g transform="translate(${padding}, ${padding})">`
  }
}
